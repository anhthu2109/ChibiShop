...
const AdminProduct = () => {
    ...
    const searchInputt = useRef(null);
    const inittial = () => ({
        name: '',
        price: '',
        description: '',
    ...
    const [stateProductDetails, setStateProductDetalis] = useState(inittial())
    ...
    const searchInputtt = useRef(null);

    const getColumnSearchProps = (dataIndex, isDate) => {
        return {
            ...
            onFilter: (value, record) => {
                if (isDate) {
                    // So sánh chỉ theo ngày mà không so sánh giờ, phút, giây
                    return moment(record[dataIndex]).isSame(value, 'day');
                }
            ...
            onFilterDropdownOpenChange: (visible) => {
                if (visible) {
                    setTimeout(() => {
                        if (searchInputt.current && searchInputt.current.input) {
                            searchInputt.current.input.select();
                        } else if (searchInputt.current && searchInputt.current.focus) {
                            searchInputt.current.focus();
                        }
                    }, 100);
                }
            },
    ...
    const dataTable = products?.data?.length && products?.data?.map((product) => {
        return { ...product, key: product._id, createdAt: product.createdAt }
    })
    ...
    useEffect(() => {
        setTotalProduct(products?.data?.length || 0);
    }, [products]);
    ...
    return (
        <div>
            ...
            <div style={{ marginTop: '20px' }}>
                <div style={{ paddingBottom: '5px' }}>Tổng số sản phẩm: {totalProduct}</div>
            ...
            <ModalComponent forceRender title="Tạo sản phẩm" open={isModalOpen} onCancel={handleCancel} footer={null}>
                <Loading isPending={isPending}>
                    ...
                    >
                        ...
                        >
                            <Select name="type" value={stateProduct.type} onChange={handleChangeSelect} options={renderOptions(typeProduct?.data?.data)} />
                        </Form.Item>
